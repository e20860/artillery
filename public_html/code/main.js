...
function main() {
   ...
   /**
    * Определяет, является ли параметр числом
    * @param {numeric} n - проверяемое значение
    * @return {logical} 
    */
    ...
    * @param {string} charge - заряд - "f","r","1","2","3","4"
    * @param {numeric} dist - дальность в целых километрах
    * @return {object} объект, представляющий собой строку поправок ТС 
    */
   ...
   //----------------------------------------------------------------------------------------------
   /**
    * определяет поправки методом линейной интерполяции
    * @param {numeric} d1   первое опорное значение (дальность в м) 
    * @param {numeric} d2   второе опорное значение (дальность в м)
    * @param {numeric} cor1 функция первого значения (поправка на d1)
    * @param {numeric} cor2 функция второго значения (поправка на d2)
    * @param {numeric} dt   значение, для которго вычисляется интерполяция (Дт)
    * @return {numeric} вычисленное значение
    */
   function interpol(d1,d2,cor1,cor2,dt){
      var scale = (cor2-cor1)/(d2-d1);
      return cor1 + (dt - d1)*scale;
   }
   //----------------------------------------------------------------------------------------------
   /**
    * Преобразует дирекционный угол в строковом представлении (41-25) в радианы
    * @param {string} al -  дирекционный угол в строковом представлении (41-25)
    * @return {numeric} представление угла в радианах
    */
   ...
   //---------------------------------------------------------------------------------------------
   /**
    * преобразует угол (в радианах) в деления угломера в строковом представлении 
    * @param {numeric} a угол в радианах 3.14159
    * @return {string} представление дирекционного угла "41-25"
    */
    ...
    * @param {point} pnt точка относительно которой решается ПГЗ
    * @param {polar} pol полярные координаты вычисляемой точки
    * @return {coords} прямоугольные координаты вычисленной точки
    */
    ...
    * решает обратную геодезическую задачу
    * @param {Coords} tgt - прямоугольные координаты целевой точки (цель)
    * @param {Coords} fp - прямоугольнык координаты точки, для которой  решается ОГЗ (ОП)
    * @return {Polar} - полярные координаты с fp на tgt 
    */
   function igp(tgt,fp) {
   ...
   //**********************************************************************************************
   /**
    * Создаёт объект класса Du (дирекционный угол)
    * @constructor
    * @this {Du}
    * @param {numeric/string} bd Если цифра - то большие деления, если строка - 
    *        угол в строковом выражении в формате "41-25" разделённом дефисом
    * @param {numeric} md малые деления дирекционного угла
    */
   function Du(bd,md) {
      // Варианты ввода:
      // 1 "45-10"
   ...
   function scCorrections(dist,dx,dz) {
      ...
      this.dz = dz;
      this.toString = function(){
          return 'Д= ' + this.dist *1000 + 'м, dД= ' + this.dx.toFixed() + 'м., dz = ' + this.dz.toFixed() + ' дел. угл;';
      }
   } //sccCorrections
   //***********************************************************************************************
   ...
   function Cop(name,crd,unit) {
      ...
      this.targetNotch = function (name,pc, num, type, fr, dp) {
         // засечка цели pc - polar coordinates
         // возвращает объект target
         return new Target(name, dgp(this.coords,pc), num,type,fr,dp);
      }
   ...
   function FirPos(name,crd,unit) {
      ...
      this.unit = unit;
      var mfd = new Du('00-00'); // main firing direction Основное направление стрельбы 0 - по умолчанию
      ...
      function outData(pc,typeData) {
          // Вывод топо- или исчисленных данных 
          // pc - полярные координаты
          // typeData - тип данных для вывода: строка "т" или "исч"
          //-------------------------------------------------------------------
          var turn = pc.alpha.val() - mfd.val();
          var eps = String(pc.e);
          eps = eps.length == 2 ? '0-' + eps: '0-0' + eps;
          eps =  (pc.e < 0) ?'-' + eps: '+' + eps;
          var turnDir = turn > 0 ? 'правее ': 'левее ';
          return 'Д' + typeData + '= ' + pc.dist.toFixed() + '; ОН, ' + turnDir + convertRadToAlpha(Math.abs(turn)) + '; е = ' + eps;
      }
      //
      //сеттеры
      ...
      this.viewCors = function() {
         for (var i = 0; i < cors.length; i++) {
            console.log(cors[i].toString());   
         }
      } // this.viewCors
      this.getTopoData = function(tgt) {
          // Расчёт топографических данных для стрельбы батареи
          var pol = igp(tgt.coords, this.coords);
          return outData(pol,'т'); 
      } //getTopoData
      this.getFireData = function(tgt) {
         // Определение исчисленной дальности и доворота для стрельбы
         var pol = igp(tgt.coords, this.coords); // топографическая дальность и дир угол по цели
         // рассчитаем поправки дальности и направления
         if (cors.length < 2) return outData(pol,'т'); // графика нет, парится не надо
         if (pol.dist < cors[0].dist * 1000 || pol.dist > cors[cors.length-1].dist * 1000) {
             return outData(pol,'т'); // дальность за пределеми графика. не считаем,
             // хотя можно и забубенить экстраполяцию
         }
         var lb = -1; // low border = нижняя граница
         for (var i = 0; i < cors.length; i++) {
             // ищем нижнюю границу интерполяции
             if (pol.dist > cors[i].dist*1000) lb = i;
         }
         if (lb <0) return outData(pol,'т');
         var d0, d1;
         d0 = (cors[lb].dist * 1000-cors[lb].dx);
         d1 = (cors[lb+1].dist * 1000 -cors[lb+1].dx)
         var deltaD = interpol(d0,d1,cors[lb].dx,cors[lb+1].dx,pol.dist);
         var deltaZ = interpol(d0,d1,cors[lb].dz,cors[lb+1].dz,pol.dist);
          // с дальностью -просто:
         pol.dist += deltaD;
         // с дирекционным углом придётся повозиться
         if (deltaZ > 0) {
             pol.alpha.add(0,deltaZ);
         } else {
             pol.alpha.sub(0,deltaZ * -1);
         }
         return outData(pol,'исч');
      }  // getFireData
  ...
  // поправки
  var m11 = new Mbul();
  m11.compose(new Date, 300,757,-5,38,6);
  
  fp5.calcCorrections('2',[3,5,8],m11);
  fp5.viewCors();
  
  // Цель 101-я
  var tgt101 = new Target('пехота', dgp(knp5.coords,new Polar('14-10',2280,5)),'101','inf01', 300,200);
  console.log(tgt101.toString());

  console.log(fp5.getTopoData(tgt101));
  console.log(fp5.getFireData(tgt101));  
  
  // Цель 102-я
  var tgt102 = knp5.targetNotch('пехота', new Polar('10-95', 3310, 12),'102','inf01',300,200);
  console.log(tgt102.toString());
  
  //Вывод топоданных по цели относительно ОП-5
   
  console.log(fp5.getTopoData(tgt102));
  console.log(fp5.getFireData(tgt102));
